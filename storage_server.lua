...
function ServerLib:UpdateInfo()
  ...
  local crop_0 = math.floor(RRL.getAnalogInput("front") / 15 * 100)
  table.insert(self.storageList,{name = "[0]Crop", value = crop_0})
  
  local wood_0 = math.floor(RRL.getAnalogInput("right") / 15 * 100)
  table.insert(self.storageList,{name = "[0]Wood", value = wood_0})
  
  local prod_0 = math.floor(RRL.getAnalogInput("back") / 15 * 100)
